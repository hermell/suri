...
# 404 에러 발생 시 suri.urls 에서 pages.views.handler404를 호출
# 404 페이지를 직접 보고 싶다면 manage.py의 debug 모드를 False로 변경
def handler404(request):
    response = render_to_response('error_pages/pages-404.html', {'path': request.path},context_instance=RequestContext(request))
...
def handler500(request):
    response = render_to_response('error_pages/pages-500.html', {'path': request.path},context_instance=RequestContext(request))

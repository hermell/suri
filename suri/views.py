...
# 404 에러 발생 시 suriProject.urls 에서 suri.views.handler404를 호출
# 404 페이지를 직접 보고 싶다면 manage.py의 debug 모드를 False로 변경
def handler404(request):
    response = render_to_response('error/pages-404.html', {'path': request.path},
...
def handler500(request):
    response = render_to_response('error/pages-500.html', {'path': request.path},
                                  context_instance=RequestContext(request))
...
def test(request):
    return render(request, 'sample/pages-404.html', {})
...
"""
urls.py에 설정되지 않는 모든 패턴에 대해 go_others에서 처리

1. request.path를 GET
2. request.path의 맨 앞에 있는 '/' 제거(e.g: /index -> index)
3. 제거된 패턴을 파라미터로 넣어 render 함수 실행값 return
"""
